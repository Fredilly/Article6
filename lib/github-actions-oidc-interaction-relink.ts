...
const EXPECTED_WORKFLOW_REF = "Fredilly/Article6/.github/workflows/crm-link-interaction-contact-v2.yml@refs/heads/main";

interface JwtHeader { alg?: string; kid?: string; }
interface GitHubJwk extends JsonWebKey { kid?: string; }
interface GitHubActionsClaims {
  iss?: string; aud?: string | string[]; exp?: number; nbf?: number; iat?: number;
  repository?: string; repository_owner?: string; actor?: string; event_name?: string;
  ref?: string; workflow_ref?: string;
}
...
export async function verifyInteractionRelinkOidc(token: string): Promise<void> {
  ...
  const valid = verify("RSA-SHA256", Buffer.from(`${encodedHeader}.${encodedPayload}`), key, Buffer.from(encodedSignature, "base64url"));
