...
import { listSalesProcurementProfiles, type SalesProcurementProfile } from "../../../lib/sales-procurement";
import { SALES_EXPERIMENTS } from "../../../lib/sales-memory";

interface Props { details: SalesOrganizationDetail[]; organizations: SalesOrganization[]; tenderOpportunities: SalesTenderOpportunity[]; procurementProfiles: SalesProcurementProfile[]; searchEntries: ReturnType<typeof buildSalesMemorySearchEntries>; initialQuery: string; initialStatus: "ALL" | SalesOrganization["status"]; }

export const getServerSideProps: GetServerSideProps<Props> = async ({ query }) => {
  const [{ details, tenderOpportunities }, procurementProfiles] = await Promise.all([
    loadSalesHomepageData(),
    listSalesProcurementProfiles(),
  ]);
  ...
  return { props: { details, organizations, tenderOpportunities, procurementProfiles, searchEntries: buildSalesMemorySearchEntries(details), initialQuery: typeof query.q === "string" ? query.q : "", initialStatus } };
};
...
export default function SalesIndexPage({ details, organizations, tenderOpportunities, procurementProfiles, searchEntries, initialQuery, initialStatus }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  return <><Head><title>Sales Memory | Article6 Internal</title><meta name="robots" content="noindex,nofollow" /></Head><SalesAutoRefresh />
    <main className="min-h-screen bg-gray-50 px-4 py-10 text-gray-900"><div className="mx-auto max-w-6xl">
      ...
      <SalesOrganizationsTable organizations={organizations} details={details} procurementProfiles={procurementProfiles} searchEntries={searchEntries} initialQuery={initialQuery} initialStatus={initialStatus} />
