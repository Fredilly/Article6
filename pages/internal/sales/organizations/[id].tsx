    ...
    <main className="min-h-screen bg-gray-50 px-4 py-10 text-gray-900"><div className="mx-auto max-w-6xl">
      ...
      <section className="mt-6 rounded-lg border border-gray-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Relationship history</h2><p className="mt-1 text-xs text-gray-500">{selectedContactId || selectedConversationId ? "Conversation view · oldest message first." : "Organization activity grouped by conversation."}</p></div><div className="text-xs text-gray-500">{conversations.reduce((count, conversation) => count + conversation.interactions.length, 0)} messages · {conversations.length} conversations</div></div>
        <div className="mt-5 space-y-6">{conversations.length ? conversations.map((conversation) => <section key={conversation.id} className="rounded-lg border border-gray-100 bg-gray-50/50 p-4"><div className="flex flex-wrap items-start justify-between gap-2 border-b border-gray-100 pb-3"><div><div className="text-sm font-semibold text-gray-900">{conversation.contactName || "Conversation"}</div><div className="mt-1 text-xs text-gray-500">{conversation.subject || "No subject"} · {conversation.interactions.length} messages</div></div>{selectedConversationId === conversation.id ? <Link href={`/internal/sales/organizations/${organization.id}`} className="text-xs font-medium text-forest-700 hover:underline">All conversations</Link> : <Link href={`/internal/sales/organizations/${organization.id}?threadId=${encodeURIComponent(conversation.id)}`} className="text-xs font-medium text-forest-700 hover:underline">Open conversation</Link>}</div><div className="mt-4 space-y-5">{conversation.interactions.map((interaction) => {
          ...
          return <article key={interaction.id} className={`flex flex-col ${presentation.alignment === "right" ? "items-end" : "items-start"}`}><div className={`w-full max-w-3xl rounded-lg border px-4 py-3 ${presentation.alignment === "right" ? "border-blue-100 bg-blue-50" : "border-gray-200 bg-gray-50"}`}><div className="flex flex-wrap items-center gap-2 text-xs text-gray-500"><span>{new Date(interaction.occurredAt).toLocaleString()}</span><span>{presentation.direction} · {interaction.channel} · {interaction.interactionType}</span>{interaction.outcomeCode ? <span className="rounded bg-gray-100 px-2 py-0.5 font-medium text-gray-700">{interaction.outcomeCode}</span> : null}</div><div className="mt-1 text-sm font-medium text-gray-900">{interaction.subject || "Interaction"}</div><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-gray-700">{interaction.summary}</p></div><div className="mt-1 flex items-center gap-2 px-1 text-xs font-medium text-gray-500"><span>{presentation.actorName}</span><details><summary className="cursor-pointer text-red-700 hover:underline">Delete record</summary><form method="post" action="/api/internal/sales" className="mt-2 rounded border border-red-200 bg-red-50 p-2 text-left"><input type="hidden" name="action" value="delete_interaction" /><input type="hidden" name="organizationId" value={organization.id} /><input type="hidden" name="interactionId" value={interaction.id} /><p className="text-xs font-normal text-red-800">Warning: this permanently deletes this interaction record.</p><input required name="confirmation" pattern="[Dd][Ee][Ll][Ee][Tt][Ee]" placeholder="Type delete" className="mt-2 w-32 rounded border border-red-300 px-2 py-1 text-xs" /><button className="ml-2 rounded bg-red-700 px-2 py-1 text-xs font-medium text-white">Delete</button></form></details></div></article>;
