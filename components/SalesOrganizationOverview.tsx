...
import TenderBidderStatusBadge from "./TenderBidderStatusBadge";
import ProcurementProfilePanel from "./ProcurementProfilePanel";
...
export function CarbonOrganizationOverview({ organization, projects, interactionCount }: Omit<Props, "tenderOpportunities"> & { tenderOpportunities?: never }) {
  return <>
    ...
    <ProcurementProfilePanel organizationId={organization.id} />
  </>;
}
...
export function TenderOrganizationOverview({ organization, tenderOpportunities, interactionCount }: Omit<Props, "projects"> & { projects?: never }) {
  ...
  return <>
    <section className="mt-6 overflow-hidden rounded-lg border border-amber-200 bg-white shadow-sm">
      ...
      <div className="border-t border-amber-100"><div className="hidden grid-cols-[minmax(0,1.8fr)_minmax(0,1.1fr)_minmax(0,1.2fr)_minmax(0,0.8fr)_minmax(0,1.5fr)_minmax(0,1fr)] gap-4 bg-amber-50/50 px-5 py-2 text-xs font-medium uppercase tracking-wide text-gray-500 md:grid"><div>Tender name</div><div>Buyer</div><div>Submission deadline</div><div>Status</div><div>Bidder status</div><div>Contract value</div></div>{tenderOpportunities.length ? tenderOpportunities.map((tender) => <Link key={tender.id} href={`/internal/sales/tenders/${tender.id}`} className="grid gap-2 border-t border-amber-100 px-5 py-4 hover:bg-amber-50/40 md:grid-cols-[minmax(0,1.8fr)_minmax(0,1.1fr)_minmax(0,1.2fr)_minmax(0,0.8fr)_minmax(0,1.5fr)_minmax(0,1fr)] md:items-center md:gap-4"><div><div className="text-xs font-medium uppercase tracking-wide text-gray-500 md:hidden">Tender name</div><div className="text-sm font-semibold text-gray-900">{tender.name}</div></div><div><div className="text-xs font-medium uppercase tracking-wide text-gray-500 md:hidden">Buyer</div><div className="text-sm text-gray-700">{value(tender.buyer)}</div></div><div><div className="text-xs font-medium uppercase tracking-wide text-gray-500 md:hidden">Submission deadline</div><div className="text-sm text-gray-700">{date(tender.submissionDeadline)}</div></div><div><div className="text-xs font-medium uppercase tracking-wide text-gray-500 md:hidden">Status</div><div className="text-sm font-semibold text-amber-700">{tender.status}</div></div><div><div className="text-xs font-medium uppercase tracking-wide text-gray-500 md:hidden">Bidder status</div><div className="mt-1 md:mt-0"><TenderBidderStatusBadge status={tender.bidderStatus} showUnclassified /></div></div><div><div className="text-xs font-medium uppercase tracking-wide text-gray-500 md:hidden">Contract value</div><div className="text-sm text-gray-700">{money(tender.contractValue)}</div></div></Link>) : <div className="px-5 py-4 text-sm text-gray-500">No tender opportunities recorded.</div>}</div>
    </section>
    <ProcurementProfilePanel organizationId={organization.id} />
  </>;
}
