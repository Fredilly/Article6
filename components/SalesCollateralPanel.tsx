...
export default function SalesCollateralPanel() {
  ...
  async function edit(item:any){const displayName=prompt("Display name",item.displayName); if(displayName===null||!displayName.trim())return; const description=prompt("Description",item.description||""); if(description===null)return; const r=await fetch("/api/internal/sales-collateral",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:item.id,organizationId:item.organizationId,contactId:item.contactId,tenderOpportunityId:item.tenderOpportunityId,interactionId:item.interactionId,displayName:displayName.trim(),documentType:item.documentType,description,sentAt:item.sentAt})}); const j=await r.json(); if(!r.ok)setError(j.error||"Unable to edit collateral."); else await load();}
  ...
  return <div className="fixed bottom-4 right-4 z-40 w-[min(420px,calc(100vw-2rem))] rounded-lg border border-gray-200 bg-white shadow-xl">
    ...
    {open?<div className="max-h-[70vh] overflow-y-auto border-t border-gray-100 p-3">
      ...
      <div className="space-y-2">{items.map((x:any)=><div key={x.id} className="rounded border border-gray-100 p-2 text-xs"><div className="flex justify-between gap-2"><div><div className="font-medium">{x.displayName}</div><div className="text-gray-500">{x.documentType}{x.contactName?` · ${x.contactName}`:""}{x.tenderName?` · ${x.tenderName}`:""}</div>{x.sentAt?<div className="text-gray-500">Sent {new Date(x.sentAt).toLocaleString()}</div>:null}{x.interactionSubject?<div className="mt-1 text-gray-600">Email: {x.interactionSubject}</div>:null}{x.description?<div className="mt-1 text-gray-600">{x.description}</div>:null}</div><div className="shrink-0"><a className="text-forest-700 hover:underline" href={`/api/internal/sales-collateral?action=download&id=${encodeURIComponent(x.id)}`}>Open</a><button onClick={()=>edit(x)} className="ml-2 text-gray-600">Edit</button><button onClick={()=>remove(x)} className="ml-2 text-red-700">Delete</button></div></div></div>)}</div>
