...
export default function SalesMemorySearch({ entries, query = "", status = "ALL", onQueryChange }: { entries: SalesMemorySearchEntry[]; query?: string; status?: string; onQueryChange?: (query: string) => void }) {
  ...
  return <div ref={containerRef} className="relative mt-7">
    <input
      value={query}
      ...
      autoComplete="off"
      placeholder="Start typing a VCS ID, project, organization, contact, email, methodology…"
      className="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-base shadow-sm outline-none focus:border-forest-600 focus:ring-2 focus:ring-forest-100"
    />
    {open && query.trim().length >= 2 ? <div className="absolute z-20 mt-2 w-full overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
      {results.length ? <div className="divide-y divide-gray-100">{results.map((result) => <Link key={result.key} href={{ pathname: `/internal/sales/organizations/${result.organizationId}`, query: { q: query, ...(status !== "ALL" ? { status } : {}) } }} onClick={() => setOpen(false)} className={`block px-4 py-3 hover:bg-gray-50 ${result.doNotContact ? "bg-red-50/60" : ""}`}>
